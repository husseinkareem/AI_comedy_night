...
def run_competition():
    """Run the joke competition."""
    print("Starting the Joke Competition!")
    
    # Ask user how many jokes they want
    try:
        num_jokes = int(input("How many jokes do you want to generate? "))
    except ValueError:
        print("Invalid input. Defaulting to 3 jokes.")
        num_jokes = 3
    
    # Generate theme and jokes
    theme, jokes = generate_theme_and_jokes(num_jokes)
    if theme is None or not jokes:
        print("Failed to generate theme and jokes. Exiting.")
        return
    ...
    # Display results
    for i, (joke, score, explanation) in enumerate(zip(jokes, scores, explanations), 1):
        print(f"\nJoke {i}:")
        print(joke)
        print(f"Score: {score}")
